...
def hent_valutakurs(fra: str, til: str):
    ...
    """
    # Sjekk cache-status FØR kursen hentes
    cached_kurs, gammel_ttl = valuta_cache.hent(fra, til)
    var_cache_hit = cached_kurs is not None
    ...
    # Hent oppdatert TTL etter eventuell cache-sett
    _, ny_ttl = valuta_cache.hent(fra, til)

    return {
        "fra": fra.upper(),
        "til": til.upper(),
        "kurs": float(kurs),
        "cache": "HIT" if var_cache_hit else "MISS",
        "ttl": gammel_ttl if var_cache_hit else ny_ttl,
